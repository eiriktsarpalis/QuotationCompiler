module internal QuotationsCompiler.Transformer
...
open QuotationsCompiler.Dependencies

[<Literal>]
let moduleName = "QuotationsCompiler"
...
let convertExprToAst (expr : Expr) =
    ...
    let letBindingToParsedInput (decl : SynModuleDecl) =
        ...
        let file = ParsedImplFileInput("/quotationsCompiler.fs", false, QualifiedNameOfFile(mkIdent range0 moduleName), [],[], [modl],false)
